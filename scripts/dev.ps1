...
# 定位项目根目录（脚本在 scripts/ 下，上一级是根目录）
$ProjectRoot = Split-Path -Parent $PSScriptRoot
Set-Location $ProjectRoot

Write-Host "=== HMark Dev Environment ===" -ForegroundColor Cyan
...
if (-not $vsPath) {
  Write-Host "错误：未找到 Visual Studio / Build Tools。请安装 Visual Studio 2022 Build Tools。" -ForegroundColor Red
  exit 1
}
...
Write-Host "导入 VS 环境: $vsDevCmd" -ForegroundColor Gray
...
Write-Host "RUSTUP_HOME: $env:RUSTUP_HOME" -ForegroundColor Gray
Write-Host "CARGO_HOME: $env:CARGO_HOME" -ForegroundColor Gray
...
if (-not $allOk) {
  Write-Host "错误：工具链不完整，请检查 VS/Windows SDK 安装。" -ForegroundColor Red
  exit 1
}

Write-Host "`n启动 pnpm dev..." -ForegroundColor Cyan
pnpm dev
